...
class MainGUI:
    def __init__(self, master):
        self.parent = master
        self.parent.title("Fan's automatic ANnotation tool (FAN tool)")
        ...
        # self.spacer2 = Label(self.ctrlPanel, text='Anno. style:')
        # self.spacer2.pack(fill=X, side=TOP)
        # self.style0Btn = Button(self.ctrlPanel, text="0 No annotation", command=self.style0)
        # self.style0Btn.pack(fill=X, side=TOP)
        # self.style1Btn = Button(self.ctrlPanel, text="1 Text", command=self.style1)
        # self.style1Btn.pack(fill=X, side=TOP)
        # self.style2Btn = Button(self.ctrlPanel, text="2 Text on block", command=self.style2)
    ...
    def load_image(self, file):
        ...
        if (len(ocr.split()) == 2):
            ...
            print('OCR failed, use manual input. ')
            application_window = self.parent
            answer = simpledialog.askstring("Input", "Failed to recognise text, input manually:", parent=application_window)
            self.scalebar.config(text='%s %s' % (answer.split()[0], answer.split()[1]))
            scaleUnit = answer.split()[1]
            imt = im[720:750,0:200]
            imtg = cv2.cvtColor(imt,cv2.COLOR_BGR2GRAY)
            dst = cv2.cornerHarris(imtg,2,1,0.04)
            result = np.where(dst > 0.3 * np.amax(dst))
            ...
            dist = (max(loc)-min(loc))
            print('distance = ' + str(dist))
            scaleRatio=dist/int(answer.split()[0])
            print('scaleRatio = ' + str(scaleRatio))
            self.scaleR.config(text='%.2f px per %s' % (scaleRatio, scaleUnit))
            print('...')
            self.scalebar.config(text='Auto-scaled: ')
    ...
    def mouse_release(self, event):
        ...
        if MMode == 4 :
            #draw rect box around and find H line
            self.bboxIdList.append(self.bboxId)
        ...
        if MMode == 5:
            ...
            if style == 1:
                ...
                self.dlineIdList.append(self.bboxId)
            if style == 2:
                self.bboxId = self.canvas.create_rectangle((x1+x2)/2-50, (y1+y2)/2-16,(x1+x2)/2+50, (y1+y2)/2+14,fill="white")
                self.dlineIdList.append(self.bboxId)
                self.bboxId = self.canvas.create_text((x1+x2)/2, (y1+y2)/2,fill="darkblue",font="Arial 16 bold",
                        text= '%d ' % (dist) + scaleUnit)
                self.dlineIdList.append(self.bboxId)
        if MMode == 6:
            ...
            xd = abs(x1-x2)
            xd = xd/scaleRatio
            self.objectListBox.insert(END, 'H line (%d->%d, %d)' % (x1, x2, y2) + ': %.2f ' % (xd) + scaleUnit )
